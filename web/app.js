...
(function (root, factory) {
    ...
    class PDFViewerApplication {
        ...
        initialize(appConfig) {
            ...
            const initializedPromise = Promise.all([Preferences.get('enableWebGL').then(value => {
        ...
        get supportsFullscreen() {
            ...
            let support = !!(doc.requestFullscreen || doc.mozRequestFullScreen || doc.webkitRequestFullScreen || doc.msRequestFullscreen);
        ...
        open(file, args) {
            ...
            const result = loadingTask.promise.then(pdfDocument => {
        ...
        fallback(featureId) {
            ...
            this.fellback = true;
            const url = this.url.split('#')[0];
            this.externalServices.fallback({ featureId, url }, download => {
                if (!download) {
                    return;
                }
                this.download();
        ...
        error(message, moreInfo) {
            let moreInfoText = mozL10n.get('error_version_info', { version: pdfjsLib.version || '?', build: pdfjsLib.build || '?' }, 'PDF.js v{{version}} (build: {{build}})') + '\n';
            if (moreInfo) {
        ...
        */
        ...
        webViewerInitialized() {
            ...
            Promise.all(waitForBeforeOpening).then(() => {
                ...
                this.error(mozL10n.get('loading_error', null, 'An error occurred while opening.'), reason);
            });
        }
